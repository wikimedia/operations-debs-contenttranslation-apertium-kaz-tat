...
cat $CORPUS | apertium -d ../../ kaz-tat-morph | apertium-tagger -p -g ../../kaz-tat.prob > /tmp/kaz-tat.crp.tagged;
for i in `cat $INFILE | sed 's/^ *//g' | sed 's/ /:/g' | cut -f1 -d'$'  | sed 's/ /_/g'`; do 
	...
	# and put it in >> <<
	cat /tmp/kaz-tat.crp.tagged | grep "\/$LEMAPOS" | head -1 | sed "s/\/$LEMAPOS/<<\/$LEMAPOS/g" | python untag-corpus.py | sed 's/[A-Za-zışçğ0-9]\+<</ >>&/g' | sed 's/  / /g' | fold -s; 
